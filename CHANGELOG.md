...
### Added
- **Resolve unnamed conversations.** New `conversations resolve-names` command derives titles for unnamed conversations from their opening human message (pure, deterministic, grapheme-safe heuristic: markdown/ANSI/bidi-control sanitization, 60-grapheme word-boundary truncation; contentless openers stay unresolved rather than getting invented titles) and mirrors them to claude.ai via the spike-verified rename endpoint (`PUT .../chat_conversations/<uuid>`, one-shot, never auto-retried). Dry-run by default with an explicit content warning (proposed titles ARE derived from conversation content); `--apply` renames serially, continues past per-item failures, classifies ambiguous transport outcomes by re-reading, stops cleanly on SIGINT, and exits nonzero if anything failed. `--id`/`--limit` for staged runs; `--json`/`--query` for machine output. Local export paths deliberately keep following the remote name (a conversation stays `unnamed-<uuid>` until the remote rename lands), so incremental sync state is untouched; a post-apply notice warns that renamed conversations re-export under new directories on next sync. See `docs/superpowers/specs/2026-07-17-resolve-unnamed-conversations-design.md` and `docs/spike-results/rename-findings.md`.
...
### Fixed
- **All SDK writes were broken by a case-duplicated `Content-Type` header** (live 400 `Input should be a valid dictionary`). `requestResponse` merged auth headers (`Content-Type`) and caller headers (`content-type`) with a plain-object spread, sending both; claude.ai rejects the joined header. This broke `renameConversation` and the shipped `putProjectMemoryControls` (`projects memory push --apply`) in production -- unit tests stub `fetch` and were blind to it, and prior live smokes were dry-run only. The merge now goes through the case-insensitive `Headers` class (caller values replace auth values; `Headers`-instance and tuple-array callers now also work), with duplicate-header regression tests on both write paths. Caught by this feature's live acceptance run.
